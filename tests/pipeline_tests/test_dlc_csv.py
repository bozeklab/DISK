...
def create_project_dlc_csv(tmp_path_factory):
...
def test_create_project_dlc_csv(create_project_dlc_csv):
    project_path = (create_project_dlc_csv / project_name)
    ## THEN
    assert_file_creation_after_create_project(project_path)


@pytest.fixture(scope="session")
def prepare_data_dlc_csv(create_project_dlc_csv):
    # STEP2. PREPARE DATA
    project_path = (create_project_dlc_csv / project_name)
...
def test_prepare_data_dlc_csv(create_project_dlc_csv, prepare_data_dlc_csv):
    project_path = (create_project_dlc_csv / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    suffix, _, _ = prepare_data_dlc_csv
    assert_file_creation_after_prepare_data(dataset_path, suffix)


@pytest.fixture(scope="session")
def train_dlc_csv(create_project_dlc_csv, prepare_data_dlc_csv):
    ## NORMAL TRAINING
    # STEP3. TRAIN
    ## GIVEN
    project_path = (create_project_dlc_csv / project_name)
    suffix, indep_keypoints, merge_keypoints = prepare_data_dlc_csv
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    logger = logging.getLogger()
...
def test_train_dlc_csv(train_dlc_csv):
    model_dir, test_dir, best_epoch, last_epoch, print_every = train_dlc_csv
    ## THEN
...
def test_impute_dlc_csv(create_project_dlc_csv, train_dlc_csv):
    project_path = (create_project_dlc_csv / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    model_dir, _, _, _, _ = train_dlc_csv
