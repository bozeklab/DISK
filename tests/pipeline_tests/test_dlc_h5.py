...
project_name = 'DISK_DLC_H5'
file_format = 'dlc_h5'
data_files = glob(os.path.join(root_path, 'behavior_data/dlc_project/videos/*.h5'))
...
def create_project_dlc_h5(tmp_path_factory):
...
def test_create_project_dlc_h5(create_project_dlc_h5):
    project_path = (create_project_dlc_h5 / project_name)
    ## THEN
    assert_file_creation_after_create_project(project_path)


@pytest.fixture(scope="session")
def prepare_data_dlc_h5(create_project_dlc_h5):
    # STEP2. PREPARE DATA
    project_path = (create_project_dlc_h5 / project_name)
...
def test_prepare_data_dlc_h5(create_project_dlc_h5, prepare_data_dlc_h5):
    project_path = (create_project_dlc_h5 / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    suffix, _, _ = prepare_data_dlc_h5
    assert_file_creation_after_prepare_data(dataset_path, suffix)


@pytest.fixture(scope="session")
def train_dlc_h5(create_project_dlc_h5, prepare_data_dlc_h5):
    ## NORMAL TRAINING
    # STEP3. TRAIN
    ## GIVEN
    project_path = (create_project_dlc_h5 / project_name)
    suffix, indep_keypoints, merge_keypoints = prepare_data_dlc_h5
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    logger = logging.getLogger()
...
def test_train_dlc_h5(train_dlc_h5):
    model_dir, test_dir, best_epoch, last_epoch, print_every = train_dlc_h5
    logger = logging.getLogger()
    ## THEN
...
def test_impute_dlc_h5(create_project_dlc_h5, train_dlc_h5):
    project_path = (create_project_dlc_h5 / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    model_dir, _, _, _, _ = train_dlc_h5
