...
project_name = 'DISK_multianimal_dlc_csv_seq'
file_format = 'dlc_csv'
...
threshoold_error_score = 1000


@pytest.fixture(scope="session")
def create_project_multianimal_dlc_csv_seq(tmp_path_factory):
...
def test_create_project_multianimal_dlc_csv_seq(create_project_multianimal_dlc_csv_seq):
    project_path = (create_project_multianimal_dlc_csv_seq / project_name)
    ## THEN
    assert_file_creation_after_create_project(project_path)


@pytest.fixture(scope="session")
def prepare_data_multianimal_dlc_csv_seq(create_project_multianimal_dlc_csv_seq):
    # STEP2. PREPARE DATA
    project_path = (create_project_multianimal_dlc_csv_seq / project_name)
...
def test_prepare_data_multianimal_dlc_csv_seq(create_project_multianimal_dlc_csv_seq,
                                             prepare_data_multianimal_dlc_csv_seq):
    project_path = (create_project_multianimal_dlc_csv_seq / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    suffix, _, _ = prepare_data_multianimal_dlc_csv_seq
    assert_file_creation_after_prepare_data(dataset_path, suffix)


@pytest.fixture(scope="session")
def train_multianimal_dlc_csv_seq(create_project_multianimal_dlc_csv_seq, prepare_data_multianimal_dlc_csv_seq):
    ## NORMAL TRAINING
    # STEP3. TRAIN
    ## GIVEN
    project_path = (create_project_multianimal_dlc_csv_seq / project_name)
    suffix, indep_keypoints, merge_keypoints = prepare_data_multianimal_dlc_csv_seq
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    logger = logging.getLogger()
...
def test_train_multianimal_dlc_csv_seq(train_multianimal_dlc_csv_seq):
    model_dir, test_dir, best_epoch, last_epoch, print_every = train_multianimal_dlc_csv_seq
    logger = logging.getLogger()
    ## THEN
...
def test_impute_multianimal_dlc_csv_seq(create_project_multianimal_dlc_csv_seq, train_multianimal_dlc_csv_seq):
    project_path = (create_project_multianimal_dlc_csv_seq / project_name)
    dataset_path = (project_path / f'DISK_data/{dataset_name}')
    model_dir, _, _, _, _ = train_multianimal_dlc_csv_seq
