...
class ParentDataset(data.Dataset):
    def __init__(self,
        ...
        if 'ground_truth' in self.data_dict.keys():
            ...
            logging.info(f'[LOADING DATASET] {self.X.shape} {np.all(np.isnan(self.X[..., :0]), axis=(1, 2)).shape} {np.unique(np.all(np.isnan(self.X[..., :0]), axis=(1, 2)))}')
            self.X = self.X[~np.all(np.isnan(self.X[..., :0]), axis=(1,2))]
            self.X_gt = self.X_gt[~np.all(np.isnan(self.X_gt[..., :0]), axis=(1, 2))]
            logging.info(f'[LOADING DATASET] {self.X.shape} {self.X_gt.shape}')
        else:
