"""
Adapted from: https://github.com/FraLuca/STSGCN
"""
...
class CNN_layer(nn.Module):
    # This is the simple CNN layer, which performs a 2-D convolution while maintaining the dimensions of the
    # input (except for the features dimension)
...
class STS_GCN(nn.Module):
    ...
    Shape:
        ...
        where
            ...
            :math:`T_{in}/T_{out}` is a length of input/output sequence,
            :math:`V` is the number of graph nodes
            :in_channels: number of channels for the coordinates (default=3)
    """
    ...
    def __init__(self,
        ...
        if n_gcn_layers % 2 != 0:
            raise ValueError(f"The number of graph layers 'n_gcn_layers' should be even. Found {n_gcn_layers}")

        self.st_gcnns.append(ST_GCNN_layer(input_channels, size_layer, [1, 1], 1, input_time_frame,
                                            joints_to_consider, st_gcnn_dropout))
        size_l = int(size_layer)

        for i_layer in range(n_gcn_layers // 2 - 1):
            self.st_gcnns.append(ST_GCNN_layer(size_l, size_l // 2, [1, 1], 1, input_time_frame,
                                                joints_to_consider, st_gcnn_dropout))
            size_l = size_l // 2
        ...
        self.st_gcnns.append(ST_GCNN_layer(size_layer, output_channels, [1, 1], 1, input_time_frame,
                            joints_to_consider, st_gcnn_dropout))

        # at this point, we must permute the dimensions of the gcn network, from (N,C,T,V) into (N,T,C,V)
        self.txcnns.append(CNN_layer(input_time_frame, output_time_frame, txc_kernel_size,
                                     txc_dropout))  # with kernel_size[3,3] the dimensions of C,V will be maintained
