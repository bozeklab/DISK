    ...
    for i_repeat in range(_cfg.evaluate.n_repeat):
        ...
        with torch.no_grad():
            ...
            for ind, data_dict in tqdm.tqdm(enumerate(test_loader), desc='Iterating on batch', total=len(test_loader)):
                ...
                if ('add_missing' not in _cfg.feed_data.transforms.keys()) or np.min(_cfg.feed_data.transforms.add_missing.pad) > 0:
                ...
                if n_plots < _cfg.evaluate.n_plots:
                    ...
                    potential_indices = np.where(n_missing > 0)[0] if ('add_missing' in _cfg.feed_data.transforms.keys()) else np.arange(len(n_missing))
